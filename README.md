...
作成時に `APP_PASSWORD` の入力欄が表示されたら、職場内で共有するパスワードを設定してください。

公開後はRenderのURLをPCとスマホで開き、次の情報を入力します。

```text
ユーザー名: inventory
パスワード: Render作成時に設定したAPP_PASSWORD
```
...
Render公開版には共通パスワードによる簡易的なアクセス保護があります。パスワードはGitHubへ書き込まず、Renderの環境変数に設定してください。

患者情報や個人情報は入力しないでください。職員ごとのアカウント、管理者権限、パスワード再発行、監査ログが必要な場合は、個別ログイン機能を追加してください。
